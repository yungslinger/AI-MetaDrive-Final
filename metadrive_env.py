...
# Modified metadrive_env to conduct effective training on SAC agent
...
METADRIVE_DEFAULT_CONFIG = dict(
    ...
    # ===== Agent =====
    random_spawn_lane_index=True, #should the agent spawn in the same lane?
...
class MetaDriveEnv(BaseEnv):
    ...
    def reward_function(self, vehicle_id: str):
        ...
        #steering_range (taken from scenario_env) to control erratic steering movements
        ...
        #continuous line penalty (scenario_env) to penalize driving on lane line (may need a bump)
        if vehicle.on_white_continuous_line:
